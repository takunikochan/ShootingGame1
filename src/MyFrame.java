...
public class MyFrame extends Frame implements Runnable {
    ...
    /*
     * public void autoSave() { final Object t=this; Runnable r = new Runnable()
     * { public void run() { try { for (int i=1 ; i<=5; i++) {
     * Thread.sleep(1000); String pathname =
     * "screenshots"+File.separator+t.getClass().getSimpleName()+"_"+new
     * TDate().toString("yyMMdd_hhmmss")+"_"+i+".png"; saveImage(new
     * File(pathname)); } } catch (Exception e) { e.printStackTrace(); } } };
     * new Thread(r).run(); }
     */
    /**
